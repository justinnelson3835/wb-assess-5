    ...
    try {
      ...
      for (const human of humansWithAnimals) {
        ...
        for (const animal of human.animals) {
          console.log(`  - Animal: ${animal.name}, Species: ${animal.species}`);
        }
      }
    } catch (error) {
...
export async function getHumansByAnimalSpecies(species) {
    try {
      ...
      const humansWithSpecies = new Set();
  
      for (const human of humansWithAnimals) {
        for (const animal of human.animals) {
          if (animal.species === species) {
            humansWithSpecies.add(`${human.fname} ${human.lname}`);
            break; 
          }
        }
      }
  
      return humansWithSpecies;
    } catch (error) {
        console.error('Error fetching data:', error);
      return null;
    }
